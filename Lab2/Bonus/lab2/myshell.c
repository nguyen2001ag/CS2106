...
void sigtstp_callback(int signum);
void my_fg(pid_t);
...
void my_process_command(size_t num_tokens, char **tokens) {
    ...
    signal(SIGTSTP, sigtstp_callback);
...
void my_info(int option){
    ...
    if (option == 0){
        for (int i = 0; i < MAX_PROCESSES && processTable[i].pid != -1; i++){
            ...
            else if (processTable[i].status == 4){
                printf("Stopped\n");
            }
        }
    }
...
void my_quit(void) {
    ...
    my_wait_terminating();
    for (int i = 0; i < MAX_PROCESSES && processTable[i].pid != -1; i++){
            if (processTable[i].status == 2 || processTable[i].status == 4){
                kill(processTable[i].pid, SIGTERM);
                printf("Killing [%u]\n", processTable[i].pid);
            }
...
void sigint_callback(int signum){
    if (fgPID == -1) return;
    for (int i = 0; i < MAX_PROCESSES && processTable[i].pid != -1; i++){
        if (processTable[i].pid == fgPID){
            kill(processTable[i].pid, SIGINT);
            printf("[%u] interrupted\n", processTable[i].pid);
            return;
        }        
    }
}
...
void sigtstp_callback(int signum){
    ...
    for (int i = 0; i < MAX_PROCESSES && processTable[i].pid != -1; i++){
        if (processTable[i].pid == fgPID){
            kill(processTable[i].pid, SIGTSTP);
...
void my_fg(pid_t pid){
    int status;
    for (int i = 0; i < MAX_PROCESSES && processTable[i].pid != -1; i++){
        if (processTable[i].pid == pid && processTable[i].status == 4){   //Stopped process
            kill(processTable[i].pid, SIGCONT);
            printf("[%u] resumed\n", processTable[i].pid);
            processTable[i].status = 2;
            fgPID = pid;
            waitpid(processTable[i].pid, &status, WUNTRACED);
            if (WIFEXITED(status)){
                processTable[i].exitCode = WEXITSTATUS(status);
                processTable[i].status = 1;     
            }
